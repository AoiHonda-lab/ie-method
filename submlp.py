...
class subMLP(chainer.Chain):
    ...
    def __init__(self, args):
        ...
        with self.init_scope():
            ...
            self.fc3 = L.Linear(1000, self.args.out)
    ...
    def train_model(self, train_iter, test_iter, optimizer, elapsed_time, start, args):
        ...
        for epoch in range(self.args.subepoch):
            ...
            for i in range(0, len(train_iter.dataset), self.args.batch_size): # batch size loop                
                ...
                loss.backward()
                optimizer.update()
                err_temp += float(loss.data)*self.args.batch_size
                # acc_temp += float(acc.data)*self.args.batch_size
                # err_temp_ += float(loss.data)
               
            train_loss.append(err_temp/len(train_iter.dataset))
            train_acc.append(acc_temp/len(train_iter.dataset))
            ...
            # limit_train 
            max_epoch = 0
            last_train = acc_temp/len(train_iter.dataset) 
