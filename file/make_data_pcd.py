...
def main():
    ...
    noiseFlag = False
    train_all = pcd_train_all[0]
    test_all = pcd_test_all[0]

    train_box = []
    test_box = []

    # train_all_, test_all_ = shuffle(train_all, test_all)
    for i in range(len(train_all)):
        train_box.append(train_all[i])
    for i in range(len(test_all)):
        test_box.append(test_all[i])
    
    print()
    
    train_all, test_all = shuffle(train_box, test_box)
    train_all = chainer_datasets(train_box)
    test_all = chainer_datasets(test_box)
    ...
    pickle.dump(pkl_data, open("./data/mnist/pcd_mnist_2class.pkl", "wb"), -1)
